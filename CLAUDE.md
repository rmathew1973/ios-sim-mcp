...
`list_simulators`, `use_simulator`, `snapshot`, `find`, `tap`, `type_text`, `key`, `button`, `swipe`, `scroll`, `launch_app`, `terminate_app`, `open_url`, `screenshot`, `await_quiescent`, `log_start`, `log_tail`, `log_stop`, `log_clear`.

WKWebView (requires dylib injected): `webview_list`, `webview_find`, `webview_fill`, `webview_click`, `webview_eval_js`.
